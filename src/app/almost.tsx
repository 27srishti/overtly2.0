...
const LandingPage: React.FC = () => {
  ...
  const [hasCompletedTabs, setHasCompletedTabs] = useState(false);
  ...
  //         const order = [0, 1, 2]; // 0: Curated News, 1: Trend Spotter, 2: Insights
  //         const currentIndex = order.indexOf(prevTab);
  //         const nextIndex = (currentIndex + 1) % order.length;
  //         const nextTab = order[nextIndex];
          
  //         // Check if we've completed all tabs (after Insights)
  //         if (nextTab === 0 && prevTab === 2) {
  //           setHasCompletedTabs(true);
  //           setIsLocked(false);
  //         }
  ...
  // Handle scroll and tab switching
  const handleScroll = useCallback((e: WheelEvent) => {
    if (!isLocked || hasCompletedTabs) {
      return;
    }
    ...
    setProgress((prev) => {
      if (prev >= 100) {
        setActiveTab((prevTab) => {
          const order = [2,1,0]; // Same sequence as above
          const currentIndex = order.indexOf(prevTab);
          const nextIndex = (currentIndex + 1) % order.length;
          const nextTab = order[nextIndex];
  ...
  }, [isLocked, hasCompletedTabs]);

  // Set up scroll lock when section is in view
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && !hasCompletedTabs) {
          setIsLocked(true);
        } else {
          setIsLocked(false);
        }
  ...
  return (
    <section className="font-readex  ">
      ...
      <section className="max-w-screen-2xl mt-10 sm:mt-0 mx-auto z-0">
        ...
        <div 
          ref={sectionRef}
          className={`relative mt-20 p-8 mx-5 lg:mx-20 sm:px-20 shadow-sm sm:py-32 
            bg-gradient-to-b from-blue-50 to-white rounded-3xl
            ${isLocked ? 'min-h-screen overflow-hidden' : ''}`}
        >
            ...
            <div className="gap-1 sm:gap-2 mt-8 flex flex-wrap">
              {["Curated News", "Trend Spotter", "Insights"].map((label, index) => (
                <div key={index} className="relative">
                  <button
                    className="border border-[#A2BEA0] font-light text-sm rounded-full 
                      px-4 py-1.5 mb-2 relative overflow-hidden bg-white"
                  >
                    <div
                      className="absolute inset-0 h-full bg-[#A2BEA0] transition-all duration-500"
                      style={{
              ...
              {[2,1,0].map((tabIndex) => (
                ...
                >
                  {activeTab === tabIndex && (
                    ...
                    <div className="flex flex-col sm:flex-row sm:w-full gap-10 sm:gap-40">
